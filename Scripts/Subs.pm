package Subs ;
BEGIN{
    if ((!$ENV{USRDIR}) || ($ENV{USRDIR} eq '')) {
        print "The USRDIR environment variable is not set. USRDIR is the directory where cluster3 package is installed (see FACE tutorial online).\n";
        exit;
    }
}
...
use lib "$ENV{USRDIR}/lib64/perl5/site_perl/5.8.5/x86_64-linux-thread-multi/" ;
...
sub printHeatmapBars
{
    ...
    for (my $j=0; $j<@$a_ta_H; $j++)
    {
	    ...
	    print $motif, "\t", $name, "\n" ;
            ...
            print OUT $mo;
            close OUT;
            system("$firedir/SCRIPTS/weblogo/seqlogo -f ./Temp/$motif.txt -F EPS  -a -c -M -n -Y -w 5 -h 3 > ./Temp/$motif.eps");
	    ...
	    if (defined $name or $name ne "")
            {
                $p->setcolour("black");
                $p->setfont("TimesBold", 10);
                $p->text( {align => "left"}, $xbase + @$r * $w + 30 + $ew * $h / $eh, $ysize - ($ybase + $i*$h +$h/2), $m_name);
            }
            else
            {
                $p->setcolour("black");
                $p->setfont("TimesBold", 10);
		$p->text( {align => "left"}, $xbase + @$r * $w + 30 + $ew * $h / $eh, $ysize - ($ybase + $i*$h+$h/2), "-");
            }
        }
        else
        {
...
sub clusterTableRows
{
    ...
    mkdir("$pagedir/TEMP") if (! -d "$pagedir/TEMP") ;
    open O, "> $pagedir/TEMP/linkdist.row.atr" ;
    my $cnt = -1 ;
    for (my $i=0 ; $i<@$result; $i++){
        my $n = $result->[$i] ;
        my $n1 = $n->[0] ;
        $n1 = $a_ta_M->[$n->[0]][0] if $n1>=0 ;
        my $n2 = $n->[1] ;
        $n2 = $a_ta_M->[$n->[1]][0] if $n2>=0 ;
        print O $cnt, "\t", $n1, "\t", $n2, "\t", $linkdist->[$i],"\n" ;
        $cnt-- ;
    }
...
sub clusterTableCols
{
    ...
    die "The clustering failed..." if $result == undef ;
    mkdir("$pagedir/TEMP") if (! -d "$pagedir/TEMP") ;
    open O, "> $pagedir/TEMP/linkdist.col.atr" ;
    my $cnt = -1 ;
    for (my $i=0 ; $i<@$result; $i++){
	my $n = $result->[$i] ;
	my $n1 = $n->[0] ;
	$n1 = $a_ta_H->[$n->[0]] if $n1>=0 ;
	my $n2 = $n->[1] ;
	$n2 = $a_ta_H->[$n->[1]] if $n2>=0 ;
	print O $cnt, "\t", $n1, "\t", $n2, "\t", $linkdist->[$i],"\n" ;
	$cnt-- ;
    }
